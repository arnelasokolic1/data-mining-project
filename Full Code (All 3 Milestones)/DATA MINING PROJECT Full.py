...
sns.barplot(y=overview_comparison['Series_Title'], x=overview_comparison['Overview_Length'], palette='coolwarm')

plt.title('Comparison of Longest and Shortest Overview Movies')
...
# Create a list of actors per movie
df[['Star1', 'Star2', 'Star3', 'Star4']] = df[['Star1', 'Star2', 'Star3', 'Star4']].fillna('')
df['Actor_List'] = df[['Star1', 'Star2', 'Star3', 'Star4']].values.tolist()

# Convert into transaction format
te = TransactionEncoder()
te_ary = te.fit(df['Actor_List']).transform(df['Actor_List'])
df_actors = pd.DataFrame(te_ary, columns=te.columns_)

#  Apply Apriori with a lower support value
frequent_actor_sets = apriori(df_actors, min_support=0.005, use_colnames=True)

# Extract association rules
if not frequent_actor_sets.empty:
    actor_rules = association_rules(frequent_actor_sets, metric="lift", min_threshold=1)
    print("\nActor Association Rules:\n", actor_rules)
else:
    print("No frequent actor pairs found. Try lowering the support value.")
...
X = df[['IMDB_Rating', 'Runtime']].dropna()  # Remove missing values
...
get_ipython().system('pip install statsmodels matplotlib seaborn')
...
# Step 1: Convert 'Released_Year' to numeric
df['Released_Year'] = pd.to_numeric(df['Released_Year'], errors='coerce').dropna().astype(int)

# Step 2: Compute the average IMDb rating per year
ratings_over_time = df.groupby('Released_Year')['IMDB_Rating'].mean()

# Step 3: Plot IMDb rating trends over time
plt.figure(figsize=(12, 6))
...
# Step 1: Decompose the time series data
decomposition = seasonal_decompose(ratings_over_time, model='additive', period=10)

# Step 2: Plot the trend, seasonality, and residuals
plt.figure(figsize=(12, 8))
...
# Select features for LOF analysis
X = df[['IMDB_Rating', 'Runtime', 'No_of_Votes']].dropna()
...
# Find outliers (LOF Score = -1 means it's an outlier)
popularity_outliers = df[df['LOF_Score'] == -1]
print("Outlier Movies Based on Popularity:")
print(popularity_outliers[['Series_Title', 'No_of_Votes', 'IMDB_Rating']])
